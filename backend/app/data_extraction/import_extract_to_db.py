...
for file_to_extract in file_paths:
    try:
        ...
        df = df.rename(columns={
            ...
            "Partner Name": "import_from",
            "Year": "year",
            "Product Group": "product",
            "Import (US$ Thousand)": "import_value_usd_thousand"
        })

        df['country_code'] = df['country'].apply(get_country_code)
        df['import_country_code'] = df['import_from'].apply(get_country_code)

        df_to_insert = df[['country_code', 'country', 'import_from', 'import_country_code', 'year', 'product', 'import_value_usd_thousand']]
...
df_all.to_sql('imports', engine, if_exists='append', index=True, index_label='id')
